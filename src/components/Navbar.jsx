...
const Navbar = () => {
    ...
    <header className="fixed top-0 left-0 w-full bg-[#A1410F] shadow-md z-50 px-4 md:px-16 py-4">
      <div className="flex items-center justify-between">
        ...
        <nav className="hidden md:flex space-x-8 text-white text-lg font-light">
          <Link to="/" className="hover:text-gray-300" data-aos="fade-down">
            Home
          </Link>
              ...
              <div className="absolute top-full left-0 w-56 mt-0 bg-[#A1410F] rounded-xl shadow-lg z-50">
                {products.map((item, index) => (
                  <Link
                    key={index}
                    to={item.link}
                    onClick={() => setShowDropdown(false)} // 👈 add this
  ...
  >
    {item.label}
          ...
          <Link to="/about" className="block hover:text-gray-300" onClick={() => setMobileMenu(false)}>About</Link>
<Link to="/contact" className="block hover:text-gray-300" onClick={() => setMobileMenu(false)}>Contact</Link>
